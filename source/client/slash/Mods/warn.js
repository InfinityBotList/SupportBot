...
module.exports = {
    ...
    run: async (client, interaction) => {
      ...
      let cases = await CASES.find({ user: member.user.id, guild: interaction.guild.id });
      ...
      let BotWarn = new client.Infinity_Gateway.MessageEmbed()
        ...
        .setColor(client.color)
        .setThumbnail(client.logo)
        .setDescription("Warnings don't count towards bots you are better off to just ban or kick them!")
        .setTimestamp()
        .setFooter({ text: client.footer, iconURL: client.logo })

      if (member == interaction.member) return interaction.reply({ embeds: [SelfWarn], ephemeral: true });

      if (member.user.bot) return interaction.reply({ embeds: [BotWarn], ephemeral: true });
      ...
      if (cases && cases.length == 3) {
            ...
            let embed = new client.Infinity_Gateway.MessageEmbed()
              ...
              .setDescription("Whoops, seems like this user has received to many warnings in this guild!")
              .addFields(
                {
                    name: "User",
              ...
              await interaction.guild.members.ban(member, { reason: "Received 3 warnings or more!" });
