    ...
    run: async(client, message) => {
        ...
        if (cmd && client.slash.get(cmd)) {
            ...
            return client.interaction.reply({
                embeds: [(await SendSlashCmdHelpEmbed({
                    client: client,
            ...
            const components = (state) => [
              ...
              const initialMessage = await message.reply({ embeds: 
                [(await SendSlashHelpEmbed({ name: 'base', client: client }))], 
              ...
              collector.on("collect", async (interaction) => {
                if (interaction.values[0] === "info") {
                    interaction.update({ embeds: [(await SendSlashHelpEmbed({ name: 'info', client: client }))], components: components(false) }).catch(() => {});
                } else if (interaction.values[0] === "about") {
                  interaction.update({ embeds: [(await SendSlashHelpEmbed({ name: 'about', client: client }))], components: components(false) }).catch(() => {});
                } else if (interaction.values[0] === "mods") {
                  interaction.update({ embeds: [(await SendSlashHelpEmbed({ name: 'mods', client: client }))], components: components(false)}).catch(() => {});
                } else if (interaction.values[0] === "fun") {
                    interaction.update({ embeds: [await SendSlashHelpEmbed({ name: 'fun', client: client })], components: components(false)}).catch(() => {});
                } 
              })
