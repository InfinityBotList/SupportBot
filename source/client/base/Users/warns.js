...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        let no_member = new client.Infinity_Gateway.MessageEmbed()
         ...
         .setDescription("Please mention the user you want to check a warnings for")
         .setTimestamp()
         .setFooter({ text: client.footer, iconURL: client.logo })

         if (!member) return message.reply({ embeds: [no_member] });
         let cases = await CASES.find({ user: member.user.id, guild: message.guild.id, action: "Warn" });
