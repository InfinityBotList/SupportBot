...
module.exports = {
  ...
  run: async (message, args, client) => {
    ...
    let embed = new client.Infinity_Gateway.MessageEmbed()
     ...
     await url.get(`https://spider.infinitybots.gg/bots/${member.user.id}`).then(async (body) => {

     let role = await message.guild.roles.cache.find(c => c.id === "870952645811134478");
     ...
     if (member.roles.cache.has(role.id)) return message.reply({ embeds: [embed3] });
     ...
     let embed = new client.Infinity_Gateway.MessageEmbed()
          ...
          .setColor("RED")
          .setThumbnail(client.logo)
          .setDescription("**Bot** is not in queue or not listed on our website")
          .setTimestamp()
          .setFooter({ text: client.footer, iconURL: client.logo })
