...
module.exports = {
    ...
    run: async (message, args, client) => {
        ...
        let no_case = new client.Infinity_Gateway.MessageEmbed()
         ...
         .setColor("RED")
         .setThumbnail(client.logo)
         .setDescription("Please provide a valid case number (ex: ``<<case @Toxic Dev 2``)")
         .setTimestamp()
         .setFooter({ text: client.footer, iconURL: client.logo })
        ...
        let caseFetch = await CASES.findOne({ user: member.user.id, guild: message.guild.id, case: caseNum });
        
        let embed = new client.Infinity_Gateway.MessageEmbed()
         .setTitle("ERROR: No cases found")
         .setColor("RED")
         .setThumbnail(client.logo)
         .setDescription("No case with that number exists for this user in this guild!")
         .setTimestamp()
         .setFooter({ text: client.footer, iconURL: client.logo });
        ...
        if (!caseFetch) return message.reply({ embeds: [embed] });
            ...
            let embed = new client.Infinity_Gateway.MessageEmbed()
             ...
             .addFields(
                ...
                {
                    name: "Moderator",
                    value: `${client.users.cache.get(caseFetch.moderator).tag}`,
                    inline: true
                },
                {
                    name: "Action Guild",
                    value: `${client.guilds.cache.get(caseFetch.guild).name}`,
                    inline: true
                },
                {
                    name: "Action Time",
