        ...
        async execute(member, client) {
            try {
                if (member.guild.id === "870950609291972618") {
                    let mainGuild = await client.guilds.cache.get(
                        "758641373074423808"
                    );
                    let guildUser = await mainGuild.members.cache.get(
                        member.user.id
                    );
                    ...
                    if (!guildUser && !member.user.bot) {
                        ...
                        if (!staffRole) {
                            ...
                            return member.guild.members.ban(member, {
                                reason: "User attempted to join a Infinity Bots Staff Server but does not appear to be staff",
                            });
                        }
                    } else {
                        if (!member.user.bot) {
